...
def sp_dmu_cos2(gphys_smn)
  ...
  sval = NArray.complex(imax,nmax+1,1)
...
def sh_divergence(u, v, a=1.0, only=false)
  ...
  #グリッド→スペクトル（経度微分：d/dlon）（緯度微分：d/dmu）
#	su = sh_trans( ucoslat, 1, false, only)
#	sv = sh_trans( vcoslat, false, 1, only)
	#スペクトル→グリッド
#	u_dlon = sh_invtrans(su, pmn, only)
#	v_dlat = sh_invtrans(sv, pmn, only)
...
def sh_vor_and_div(u, v, a=1.0, only=false)
	...
	#グリッド→スペクトル
  # su = sh_trans( ucoslat, false, false, only)
  # sv = sh_trans( vcoslat, false, false, only)
...
#
def sh_energyspectrum(u, v, a=1.0, comp="total" )
# 参照：Burgess, Erler, and Shepherd (2013) The Troposphere-to-Stratosphere Transition in Kinetic Energy Spectra and Nonlinear Spectral Fluxes as Seen in ECMWF Analyses, JAS, vol. 70, pp. 669-687
	...
	when "div", "div_u", "div_v" then
		...
		shapes = sdiv.shape
		sdivval = sdiv.val
	when "total", "total_u", "total_v" then
		vor = sh_vorticity( u, v, a )
		svor = sh_trans(vor)
		svorval = svor.val
		div = sh_divergence( u, v, a )
		...
		raise "the 7th argument must be 'total' (default), 'total_u', 'total_v', 'rot', 'rot_u', 'rot_v', 'div', 'div_u', or 'div_v'."
	end
